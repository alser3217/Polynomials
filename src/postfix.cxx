...
Polynomial calculate_function(Polynomial a, Polynomial b, std::string _operator) {
  Polynomial result;
  int value = 0;
  std::map<std::string, int> mp = { {"*", 1}, {"+", 3}, {"-", 4} };
  for (const auto& kv : mp)
    if (_operator == kv.first) {
      value = kv.second;
      break;
...
Polynomial TPostfix::calculate(std::vector<Polynomial> polynomials) {
  bool flag = false;
  std::string operations[3] = {"+", "-", "*"};
  int j = 0;
  TStack<Polynomial> operands(MAX_STACK_SIZE);
  for (int i = 0; i < postfix.size(); i++) {
    flag = false;
    for (std::string symbol : operations)
      if (postfix[i] == symbol) {
